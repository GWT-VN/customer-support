...
export function BaoTriDoneButton({ visitId, completedAt }: { visitId: string; completedAt: string | null }) {
  ...
  const setNum = (k: keyof KetQuaDo, v: string) => setKq((c) => ({ ...c, [k]: v === '' ? undefined : Number(v) }))

  const oNum = 'w-16 rounded border px-1.5 py-0.5 text-xs'
  if (moKQ) {
    const hang = (nhan: string, kt: keyof KetQuaDo, ks: keyof KetQuaDo) => (
      <div className="flex items-center gap-1.5">
        <span className="w-20 text-slate-600">{nhan}</span>
        <input inputMode="decimal" placeholder="trước" value={(kq[kt] as number | undefined) ?? ''} onChange={(e) => setNum(kt, e.target.value)} className={oNum} />
        <span className="text-slate-300">→</span>
        <input inputMode="decimal" placeholder="sau" value={(kq[ks] as number | undefined) ?? ''} onChange={(e) => setNum(ks, e.target.value)} className={oNum} />
      </div>
    )
    return (
  ...
  if (completedAt) {
    return (
      <div className="flex items-center gap-1.5">
        ...
        <button onClick={() => { setMoKQ(true); setKq({ ngay: completedAt.slice(0, 10) }) }} className="text-[10px] text-sky-600 underline">kết quả</button>
  ...
  if (!open) {
    return (
      <div className="flex items-center gap-1.5 flex-wrap">
        ...
        <button onClick={() => { setMoKQ(true); setKq({ ngay: HOM_NAY() }) }} className="text-xs text-sky-600 underline">+ kết quả đo</button>
        {msg && <span className="text-[10px] text-emerald-700">{msg}</span>}
        {err && <span className="text-xs text-red-600">{err}</span>}
      </div>
    )
