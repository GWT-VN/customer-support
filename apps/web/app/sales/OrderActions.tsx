...
export function OrderActions({ orderCode }: { orderCode: string }) {
  ...
  function onDelete() {
    if (!confirm(`Xoá đơn ${orderCode}? Không hoàn tác được.`)) return
    setErr(null)
    start(async () => {
      const res = await xoaDon(orderCode)
      if (res.ok) {
        router.push('/sales')
        router.refresh()
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <div className="flex items-center gap-2">
        <Link href={`/sales/don/${encodeURIComponent(orderCode)}/sua`} className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-50">Sửa</Link>
