...
export default async function NhanSuKyThuatPage() {
  ...
  const [dsKt, quyen, trangThai] = await Promise.all([
    dsKyThuat(false),
    hoiQuyen({ taiKhoanKT: ['cs.ky_thuat.tai_khoan', 'ADMIN'] }),
    trangThaiTaiKhoanKT(),
  ])
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-4">
        <header className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-sm text-slate-500">Nhân viên + cộng tác viên đi hiện trường. {quyen.taiKhoanKT ? 'Cấp đăng nhập cho kỹ thuật bằng email của họ (họ chỉ thấy lịch chuyến của mình).' : 'Chỉ quản trị mới cấp được quyền đăng nhập.'}</p>
          </div>
          <Link href="/ky-thuat" className="rounded-lg border px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-50">Gán lịch →</Link>
        </header>

        <RosterKyThuat dsKt={dsKt} trangThai={trangThai} choTaiKhoanKT={quyen.taiKhoanKT} />
      </div>
    </main>
  )
}
